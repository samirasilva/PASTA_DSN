...
bool PatientModule::getPatientData(services::PatientData::Request &request, 
    ...
    // vital signs = oxigenation, heart_rate, temperature, abps, abpd, glucose
    ...
    if (request.vitalSign == "oxigenation") {
        lowest_value = 0;
        highest_value = 100;         
    } else if (request.vitalSign == "heart_rate") {
        lowest_value = 0;
        highest_value = 300; 
    } else if (request.vitalSign == "temperature") {
        lowest_value = 0;
        highest_value = 50;          
    } else if (request.vitalSign == "abps") {
        lowest_value = 0;
        highest_value = 300;          
    } else if (request.vitalSign == "abpd") {
        lowest_value = 0;
        highest_value = 300;        
    } else if (request.vitalSign == "glucose") {
        lowest_value = 20;
        highest_value = 200;         
    }
    ...
    std::mt19937 seed(rd());
    std::uniform_int_distribution<int> probabilityGenerator(lowest_value,highest_value);
    response.data = probabilityGenerator(seed);
